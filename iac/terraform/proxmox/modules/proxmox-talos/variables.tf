...
variable "network_interface_index" {
  description = "Index of the network interface to read IP addresses from in the QEMU guest agent"
  type        = number
  default     = 7
}
...
variable "cluster_endpoint" {
  description = "Optional VIP or load balancer address for the Kubernetes API endpoint. Falls back to the first control node IP if not set."
  type        = string
  default     = null
}
